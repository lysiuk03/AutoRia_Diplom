...
const ImageGallery: React.FC<ProductInfoProps> = ({ car }) => {
    ...
    const images  = photos.length > 0 ? photos.map(photo => photo) : [];
    ...
    return (
        <div className="photo">
            <div className="main-photo">
                ...
                {images.length > 0 ? (
                    <img src={`http://localhost:5174/images/1200_${images[currentIndex].name}`} alt="Car" />
                ) : (
            ...
            <div className="thumbnail-gallery">
                {images.map((image, index) => (
                    <img
                        key={index}
                        src={`http://localhost:5174/images/1200_${image.name}`}
                        alt={`Thumbnail ${index}`}
